...
%   Detailed explanation goes here
  ...
  for pos = 1:left_orig
    ...
    Blocks.Right(end) = [];
    LeftMPSMatrix_full = reshape(eye(dim_l*dim_q),[dim_l,dim_q,dim_l*dim_q]);
    myLeftBlock = renorm(Blocks.Left{pos},Model.MPO{pos},Model.MPOlogical{pos},LeftMPSMatrix_full,"SIDE",'left');
    ...
    if options.KeepPSI0
      LeftMPSMatrix_new = orth_keepPSI0(PSI0_LR,PSInew_LR);
    else
      [LeftMPSMatrix_new,~] = qr([PSI0_LR,PSInew_LR],'econ');
    end
    LeftMPSMatrix_new = reshape(LeftMPSMatrix_new,[dim_l,dim_q,size(LeftMPSMatrix_new,2)]);
