function block_out = renorm(block_in, MPOmatrix, MPOmatrix_logical, MPSmatrix, options)
...
arguments
  ...
  MPSmatrix (:,:,:) double;   %block_in local block_out
  ...
  switch options.SIDE
    case 'left'
      ...
      for taskID = 1:length(MPOrow)
        ...
        tmp = tensorprod(OP_block_in,MPSmatrix,2,1,'NumDimensionsA',2);
        tmp = tensorprod(tmp,OP_loc,2,2,'NumDimensionsA',3);
        tmp = tensorprod(conj(MPSmatrix),tmp,[1,2],[1,3],'NumDimensionsA',3);
